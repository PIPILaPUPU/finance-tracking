...
func (s *AccountService) UpdateName(
	ctx context.Context,
	userID uuid.UUID,
	accountID uuid.UUID,
	...
	updated, err := s.rep.UpdateName(ctx, userID, accountID, req)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) || errors.Is(err, repository.ErrNotFound) {
			return model.Account{}, ErrAccountNotFound
		}
	...
	if updated.ParentID != nil && updated.AllocationRule == model.AllocationPercent && updated.Percent != nil {
		parent, err := s.rep.GetById(ctx, userID, *updated.ParentID)
		if err != nil {
			if errors.Is(err, pgx.ErrNoRows) || errors.Is(err, repository.ErrNotFound) {
				return model.Account{}, ErrAccountNotFound
			}
...
func (s *AccountService) UpdateSubAccount(
	ctx context.Context,
	userID uuid.UUID,
	accountID uuid.UUID,
	req model.UpdateSubAccountRequest,
) (model.Account, error) {
	req.AllocationRule = strings.TrimSpace(strings.ToLower(req.AllocationRule))
	if req.AllocationRule == "" {
		req.AllocationRule = model.AllocationManual
	}
	...
	if account.ParentID == nil {
		return model.Account{}, fmt.Errorf("%w: only sub-accounts support allocation update", ErrInvalidRequest)
	}
	...
	createLike := model.CreateAccountRequest{
		Name:           account.Name,
		Balance:        req.Balance,
		AllocationRule: req.AllocationRule,
		Percent:        req.Percent,
	}
	if err := validateSubRequest(createLike, req.AllocationRule); err != nil {
		return model.Account{}, fmt.Errorf("%w: %s", ErrInvalidRequest, err.Error())
	}

	balance, percent, err := resolveSubBalance(parent.Balance, req.AllocationRule, createLike)
	if err != nil {
		return model.Account{}, fmt.Errorf("%w: %s", ErrInvalidRequest, err.Error())
	}
	...
	updated, err := s.rep.UpdateSubAccount(ctx, userID, accountID, model.Account{
		Balance:        balance,
		AllocationRule: req.AllocationRule,
		Percent:        percent,
	})
